...
function loadTemplateBodyHtml(slug) {
  const sourcePath = path.join(process.cwd(), "src-templates", "table", `${slug}.html`);
  let raw;
  try {
    ...
    if (error?.code === "ENOENT") {
      return `<div style="padding:2rem;font-family:system-ui, sans-serif;text-align:center;">
        <h1>404</h1>
        <p>Template not found: table/${slug}.html</p>
      </div>`;
    }
    throw error;
  }
  ...
  if (!bodyMatch) return `<div>Missing &lt;body&gt; in table/${slug}.html</div>`;
...
export default async function TableRoute({ params }) {
